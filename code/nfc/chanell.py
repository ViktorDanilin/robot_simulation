...
def main():
   ...
   pub2=rospy.Publisher('/chanell',String,queue_size = 16)
   ...
   while not rospy.is_shutdown() and ch == '' and continue_reading:
      try:
          print "LOOP running"
          ...
          if status == MIFAREReader.MI_OK:
              ...
              if status == MIFAREReader.MI_OK:
                  ...
                  if back_data != None:
                      ...
                      pub.publish(fat)
                      ch = 'AT+C'+str(input('Select chanell'+'\n'))
                      text = String(data=ch)
                      #print('This chanell you take'+text)
                      pub2.publish(text)
                      r.sleep();
